...
Score::Score(const char* n, int ae, int sc, time_t t)
    : applesEaten(ae), score(sc), date(t)
{
...
{
    time_t now = time(0);
    struct tm* ptr2tm = localtime(&now);
	int month = ptr2tm->tm_mon + 1;
	std::string datestring = std::to_string(month);
	datestring += '/';
...
std::ostream& operator<<(std::ostream& out, const HighScores& scores)
{
    ...
    else
    {
        ...
        out << "     Name           Score    Apples      Date" << std::endl;
        auto count = 1;
