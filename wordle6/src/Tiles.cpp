...
Tiles::Tiles()
{
    ...
    for (auto row = 0U; row < Rows; row++)
    {
        ...
        {
            location = sf::Vector2f(0.219f*GameSize.x+TileWidth/2.0f+col*(TileWidth + TileBorder),0.19f*GameSize.y+row * (TileHeight + TileBorder));
...
void Tiles::resetRowPosition(unsigned row)
{
    ...
    for (unsigned col = 0; col < WordSize; col++)
    {
        ...
        array[row][col]->getTile().setPosition(sf::Vector2f(xpos,ypos));
        array[row][col]->getLetter().setPosition(sf::Vector2f(xpos - 10.f,ypos - 19.f));
    }
}
