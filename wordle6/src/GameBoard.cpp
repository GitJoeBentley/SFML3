...
GameBoard::GameBoard()
    :  words(),
       window(sf::VideoMode(sf::Vector2u(720u, 768u)), "", sf::Style::Close),
       background(GameSize),
       title(TheFont, Title, 32),
...
void GameBoard::copyGameToClipboard(const TileContents& tileContents, unsigned gameNumber, unsigned numberOfGuesses)
{
    ...
    if (!screenshot.saveToFile("game_results.jpg"))
        cerr << "Unable to open file: game_results.jpg" << endl;
    delete [] ptr2Tiles;
    ptr2Tiles = nullptr;
}
...
void GameBoard::resetTileRowPosition(unsigned row)
{
    ...
    for (unsigned col = 0; col < 6; col++)
    {
        ...
        tiles.getTile(row,col)->getLetter().setPosition(sf::Vector2f(xpos-10.f,ypos-19.f));
    }
}
...
GameStatus GameBoard::processEnter(const PlayerStatus& playerStatus, GameStatus& gameStatus, Statistics& stats, unsigned& row, unsigned& col)
{
    ...
    unsigned guessEvaluation[WordSize];
