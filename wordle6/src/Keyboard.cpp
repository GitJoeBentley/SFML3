...
Keyboard::Keyboard()
{
    ...
    if (!keyTexture.loadFromFile(KeyImageFile))
        std::cerr << "Unable to load file " << KeyImageFile << std::endl;
    if (!enterTexture.loadFromFile(EnterImageFile))
        std::cerr << "Unable to load file " << EnterImageFile << std::endl;
    if (!backspaceTexture.loadFromFile(BackspaceImageFile))
        std::cerr << "Unable to load file " << BackspaceImageFile << std::endl;
