...
ButtonBox::ButtonBox(sf::RenderWindow& wdw, std::vector<std::string> buttonLabels, sf::Font& font, float relativeVerticalPos, unsigned textSize)
    : window(wdw), numButtons(buttonLabels.size()), buttons(new Button*[numButtons])
{
    ...
    setPosition(sf::Vector2f(wdw.getSize().x/2.0f, relativeVerticalPos * wdw.getSize().y));
    ...
    //float buttonBoxTop = wdw.getSize().y * relativeVerticalPos;
