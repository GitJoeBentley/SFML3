...
Tiles::Tiles()
{
    ...
    for (auto row = 0U; row < Rows; row++)
    {
        ...
        {
            location = sf::Vector2f(0.313f * GameSize.x + TileWidth / 2.0f + col * (TileWidth + TileBorder), 0.19f * GameSize.y + row * (TileHeight + TileBorder));
...
void Tiles::resetRowPosition(unsigned row)
{
    ...
    for (unsigned col = 0; col < Cols; col++)
    {
        ...
        array[row][col]->getLetter().setPosition(sf::Vector2f(xpos-10,ypos-19));
    }
}
