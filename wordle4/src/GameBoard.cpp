...
sf::Font GameBoard::TheFont(FontFile);

GameBoard::GameBoard()
    :  words(),
       window(sf::VideoMode(GameSizeU), "", sf::Style::Close),
       background(GameSize),
       title(TheFont,sf::String(Title), 32),
       line1(sf::Vector2f(GameSize.x,1.0f)),
       line2(sf::Vector2f(GameSize.x,1.0f)),
       howtoplayButtonTexture(HowToPlayButtonImageFile),
       howtoplayButton(howtoplayButtonTexture),
       statsButtonTexture(StatsButtonImageFile),
       statsButton(statsButtonTexture)
{
    ...
    title.setPosition(sf::Vector2f(GameSize.x/2.0f,0.022f*GameSize.y));
    title.setOrigin(sf::Vector2f(title.getLocalBounds().size.x/2.0f,title.getLocalBounds().size.y / 2.0f));

    line1.setFillColor(GreyColor);
    line1.setPosition(sf::Vector2f(0.0f,0.004f*GameSize.y));
    line2.setFillColor(GreyColor);
    line2.setPosition(sf::Vector2f(0.0f,0.061f*GameSize.y));

    statsButton.setPosition(sf::Vector2f(0.96f*GameSize.x,0.03f*GameSize.y));
    statsButton.setOrigin(sf::Vector2f(statsButton.getLocalBounds().size.x/2.0f,statsButton.getLocalBounds().size.y / 2.0f));

    howtoplayButton.setPosition(sf::Vector2f(0.04f*GameSize.x,0.03f*GameSize.y));
    howtoplayButton.setOrigin(sf::Vector2f(howtoplayButton.getLocalBounds().size.x/2.0f,howtoplayButton.getLocalBounds().size.y / 2.0f));
}
...
sf::Vector2f GameBoard::gethowtoplayButtonPosition() const
{
    return howtoplayButton.getPosition();
}

sf::Vector2f GameBoard::getStatsButtonPosition() const
{
...
{
    string guess;
    for (auto col = 0u; col < WordSize; col++)
        guess += tiles.getTile(row,col)->getTile().getChar();
    return guess;
}

Tile& GameBoard::tile(unsigned row, unsigned col)
{
...
void GameBoard::copyGameToClipboard(const TileContents& tileContents, unsigned gameNumber, unsigned numberOfGuesses)
{
    ...
    sf::Text line(TheFont, sf::String(str));
    ...
    {
        for (unsigned col = 0; col < WordSize; col++)
        {
    ...
    if (!screenshot.saveToFile("game_results.jpg"))
        cerr << "Unable to save game_results.jpg" << endl;
    delete [] ptr2Tiles;
    ptr2Tiles = nullptr;
}
...
{
    // Message Box
    sf::Text message(TheFont, sf::String(msg));
    message.setCharacterSize(16);
    message.setStyle(sf::Text::Bold);
    message.setFillColor(sf::Color::White);
    message.setPosition(sf::Vector2f(GameSize.x/2.0f,0.11f*GameSize.y));
    message.setOrigin(sf::Vector2f(message.getLocalBounds().size.x/2.0f,message.getLocalBounds().size.y / 2.0f));
    ...
    messageBox.setPosition(sf::Vector2f(GameSize.x/2.0f,0.11f*GameSize.y));
    message.setString(msg);
    messageBox.setSize(sf::Vector2f(1.25f * message.getLocalBounds().size.x,40.0f));
    messageBox.setOrigin(sf::Vector2f(messageBox.getLocalBounds().size.x/2.0f,messageBox.getLocalBounds().size.y / 2.0f));
...
void GameBoard::resetTileRowPosition(unsigned row)
{
    ...
    for (unsigned col = 0; col < 5; col++)
    {
        ...
        tiles.getTile(row,col)->getTile().setPosition(sf::Vector2f(xpos,ypos));
        tiles.getTile(row,col)->getLetter().setPosition(sf::Vector2f(xpos-10,ypos-19));
    }
}
...
{
    int r = rand() % 5;
    tile(targetRow,r).setLetter(words.whatsTheWord()[r]);
    for (auto i = 0; i < 1000; i++)
    {
        draw("");
        window.display();
    }
    tile(targetRow,r).setLetter(' ');
}
...
void GameBoard::howtoplay()
{
    ...
    howtoplayRect.setPosition(sf::Vector2f(0.0f,0.0f));

    sf::Text instructions(GameBoard::TheFont, getHowToPlayText(), 16);
    instructions.setFillColor(sf::Color::Black);
    instructions.setPosition(sf::Vector2f(60.0f,10.0f));

    sf::Text lastLine(GameBoard::TheFont,"One new word is available to play each day, but you can practice all you want.",16);
    lastLine.setFillColor(sf::Color::Black);
    lastLine.setPosition(sf::Vector2f(60.0f,640.0f));

    sf::Texture howtoplayexampleTexture(HowToPlayImageFile);
